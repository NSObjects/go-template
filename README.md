...
go-template 是一个基于 Echo + samber/do 的 API 服务模板，围绕 Clean Architecture 构建，提供分层脚手架、按需加载的数据访问、可观测性与工程化工具链，帮助团队快速落地可维护的 API 服务。

## ✨ 核心特性

- **分层架构**：业务、数据、接口职责清晰，方便扩展与测试
- **依赖注入**：通过 samber/do 组织组件与生命周期
- **数据访问**：集成 GORM、数据库迁移与查询封装
- **认证能力**：提供 JWT 中间件基础能力，权限系统可按项目需要独立集成
