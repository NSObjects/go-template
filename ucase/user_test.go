...
func TestUserHandler_GetUser(t *testing.T) {
	...
	pmock.EXPECT().FindUser(models.UserParam{ApiQuery: models.ApiQuery{Page: 1, Count: 10}})
...
func CreatedUserContext() echo.Context {
	...
	req.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
	rec := httptest.NewRecorder()
	e.Validator = &api_helper.Validator{Validator: validator.New()}
	return e.NewContext(req, rec)
}
...
func TestUserHandler_CreateUser(t *testing.T) {
	...
	defer ctl.Finish()
	pmock := mock_repository.NewMockUserRepository(ctl)
	pmock.EXPECT().CreateUser(models.UserParam{
		User: models.User{Id: 1, Name: "lin", Phone: "string", Account: "string", Password: "string", Status: 1}})
	type fields struct {
		repository repository.UserRepository
	}
	type args struct {
...
func TestUserHandler_UpdateUser(t *testing.T) {
	...
	defer ctl.Finish()
	pmock := mock_repository.NewMockUserRepository(ctl)
	pmock.EXPECT().UpdateUser(models.UserParam{
		User: models.User{Id: 1, Name: "lin", Phone: "string", Account: "string", Password: "string", Status: 1}}, int64(1))
	type fields struct {
		repository repository.UserRepository
	}
	type args struct {
