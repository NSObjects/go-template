...
// RateLimit 限流中间件
func (rl *RateLimiter) RateLimit(config RateLimitParams) echo.MiddlewareFunc {
...
// isAllowed 检查是否允许请求
func (rl *RateLimiter) isAllowed(ctx context.Context, key string, config RateLimitParams) (bool, error) {
	now := time.Now()
